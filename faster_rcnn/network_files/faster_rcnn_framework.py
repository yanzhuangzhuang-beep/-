...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        #images==>image_size(b,h,w),tensor(8,3,1024,1024)  target==>(b,5)boxes ,lables,image_id,area,iscrowd
        ...
        #proposals==》[b,969/880,4] proposal_losses==》loss_objectness,loss_rpn_box_reg
        proposals, proposal_losses = self.rpn(images, features, targets)

        # 将rpn生成的数据以及标注target信息传入fast rcnn后半部分(选取一部分)
        #detections==>(训练结束进行的后处理)"boxes":"labels":  "scores"  detector_losses==>"loss_classifier", "loss_box_reg"
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 ...
                 rpn_batch_size_per_image=256, rpn_positive_fraction=0.5,  # rpn计算损失时采样的样本数(正负样本数），以及正样本占总样本的比例
        ...
        #  Multi-scale RoIAlign pooling(官方的更好）
        ...
        if box_head is None:
            resolution = box_roi_pool.output_size[0]  # 默认等于7 roi_pool后转化为固定的7*7
            representation_size = 1024
            box_head = TwoMLPHead(
                out_channels * resolution ** 2,#outchannel*7*7参数个数
                representation_size
            )
