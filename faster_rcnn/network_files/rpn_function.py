...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, size, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        Arguments:
            ...
            device: cpu/gpu
        """
        scales = torch.as_tensor(size, dtype=dtype, device=device)
        aspect_ratios = torch.as_tensor(aspect_ratios, dtype=dtype, device=device)
        h_ratios = torch.sqrt(aspect_ratios)
        w_ratios = 1.0 / h_ratios
        #scale is size
        # [r1, r2, r3]' * [s1, s2, s3]
        # number of elements is len(ratios)*len(scales)  the mean is w*sqrt(0,5)*1/sqrt(0,5)
    ...
    def cached_grid_anchors(self, grid_sizes, strides):
        ...
        return anchors
    #image_list(tensor(b),image_size)
    #feature_maps预测特征层的个数
    def forward(self, image_list, feature_maps):
        ...
        # 根据提供的sizes和aspect_ratios生成anchors模板(对应image_size的尺度）
        ...
        for i, (image_height, image_width) in enumerate(image_list.image_sizes):
            anchors_in_image = []
            # 遍历每张预测特征图映射回原图的anchors坐标信息(mobilenet只有一个特征图· resNet有四个特征图)
...
class RPNHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors):
        ...
        # 计算预测的目标分数（这里的目标只是指前景或者背景）k
        self.cls_logits = nn.Conv2d(in_channels, num_anchors, kernel_size=1, stride=1)
        # 计算预测的目标bbox regression参数 4k
...
def permute_and_flatten(layer, N, A, C, H, W):
    ...
    layer = layer.view(N, -1, C,  H, W)#【8，15，1，32，32】
    # 调换tensor维度
    layer = layer.permute(0, 3, 4, 1, 2)  # [N, H, W, -1, C]
    layer = layer.reshape(N, -1, C)#【8，15360，1】
    return layer
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    for box_cls_per_level, box_regression_per_level in zip(box_cls, box_regression):
        ...
        # [N, -1, C]展平处理[8,15360,1]
        box_cls_per_level = permute_and_flatten(box_cls_per_level, N, A, C, H, W)
        box_cls_flattened.append(box_cls_per_level)

        # [N, -1, C]展平处理[8,15360,4]
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self, anchor_generator, rpnhead,
        ...
        self.proposal_matcher = det_utils.Matcher(
            ...
            allow_low_quality_matches=True#如果存在某一个GT没有匹配到anchor时是否把此anchor的最大值划入预选区
        )
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                #>=0 anchor的位置填充为gtbox
                matched_gt_boxes_per_image = gt_boxes[matched_idxs.clamp(min=0)]#[gt_2 gt_1 'gt_0' gt_0 ''gt_0'' gt_3]
                ...
                # background (negative examples)负样本的置为0
                bg_indices = matched_idxs == self.proposal_matcher.BELOW_LOW_THRESHOLD  # -1
                labels_per_image[bg_indices] = 0.0

                # discard indices that are between thresholds 丢弃的置为-1
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        #idx预测特征层索引 full(n,idx)是填充n个0=这里如果只有一个预测特征层
        ...
        for boxes, scores, lvl, img_shape in zip(proposals, objectness_prob, levels, image_shapes):
            ...
            # non-maximum suppression, independently done per level 非极大值抑制
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        # 将所有正负样本索引拼接在一起 注意每张图片正负样本数之和256 8张图片正负样本数之和2048
    ...
    def forward(self,
        ...
        #objectness[b,15,24,32]pred_bbox_deltas[b,60,24,32
        ...
        # 调整内部tensor格式以及shape
        #objectness=>[batch*anchor_number*h*w,1]
        ...
        if self.training:
            ...
            # 结合anchors以及对应的gt，计算（anchor对应GTbox回归参数）
