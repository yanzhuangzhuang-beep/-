...
class SqueezeExcitation(nn.Module):
    #第一个全连接层节点个数input_channel/4
    def __init__(self, input_c: int, squeeze_factor: int = 4):
        super(SqueezeExcitation, self).__init__()
        squeeze_c = _make_divisible(input_c // squeeze_factor, 8)
        self.fc1 = nn.Conv2d(input_c, squeeze_c, 1)#作用与全连接层一样
...
class IntermediateLayerGetter(nn.ModuleDict):
    ...
    Arguments:
        model (nn.Module): model on which we will extract the features =backbone
    ...
    def forward(self, x):
        ...
        for name, module in self.items():
            ...
            if name in self.return_layers:
                ...
                out[out_name] = x
        #out[input_channel_orginal,output_channel,h,w]
        return out
...
#train中参数norm_layer=torch.nn.BatchNorm2d,trainable_layers=3
def resnet50_fpn_backbone(pretrain_path="",
    ...
    # 记录resnet50提供给fpn的每个特征层channel  in_channels_list=[256,512,1024,2048]
